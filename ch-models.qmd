...
if (params$use_synthetic_data) {
  ...
  # Real data from the pins board on $LTA_ROOT/LTA_Board (ltabase).
  # The arguments are taken from current_sp (set in _Setup.R):
  # studyprogram = full name, studytrack = code, studyform = VT/DT/DU
  df_sp_enrollments <- 
    get_lta_studyprogram_enrollments_pin(board = pin_board,
                                         faculty = current_sp$INS_Faculteit,
                                         studyprogram = current_sp$INS_Opleidingsnaam_huidig,
                                         studytrack = current_sp$INS_Opleiding,
                                         studyform = toupper(params$sp_form),
                                         range = "eerstejaars") |> 
    mutate(
      # The real data has no ID column (the synthetic set creates one):
      # use the LTA student ID, or the row number if that is missing
      ID = if ("INS_Student_ID_LTA" %in% names(pick(everything()))) {
        as.character(INS_Student_ID_LTA)
      } else {
