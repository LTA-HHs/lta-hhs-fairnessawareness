...
if (params$data == "synthetic") {
  df_sp_enrollments <- get_sp_enrollments_syn(sp = params$sp,
...
} else if(params$data == "1cho") {
    
    opleidingsnaam <- current_sp$INS_Opleidingsnaam_huidig
    opleidingsvorm <- toupper(params$sp_form)

    install_formats("parquet")
    ...
    source("R/scripts/1cho/01_read_metadata.R")
    ...
    source("R/scripts/1cho/02_transform_data.R")
    df_sp_enrollments <- transform_data(metadata,
                         opleidingsnaam,
                         opleidingsvorm,
                         0,
                         df1cho,
  ...
  } else if (params$data == "real") {
    
  library(ltabase)

  df_sp_enrollments <- 
    ltabase::get_lta_studyprogram_enrollments_pin(board = "HHs/Inschrijvingen",
                                         faculty = params$faculty,
                                         studyprogram = current_sp$INS_Opleidingsnaam_huidig,
                                         studyform = toupper(params$sp_form),
                                         range = "eerstejaars")
    
  } else {
    stop(paste0("Data source not recognized: ", params$data, ".\n Choose between \"1cho\", \"real\"r or \"synthetic\""))
  }
...
df_sp_enrollments <- df_sp_enrollments |>
  ...
  select(any_of(c(list_select))) |>
  
  # Rename variables for more readable names
  rename_with(
  ~ dplyr::coalesce(name_mapping_list[.x], .x),
  .cols = everything()
  ) |>
...
lr_recipe <- recipe(Retentie ~ ., data = df_retention_train) |>
  update_role(ID, new_role = "ID") |>
  step_rm(ID, Collegejaar)

if (params$data != "1cho") {
  lr_recipe <- lr_recipe |>
    step_unknown(Studiekeuzeprofiel, new_level = "Onbekend skp")
}
